 <div class="row">
        ...
        <div class="col-md-6 col-lg-9">
            <select class="custom-select d-block w-100" id="warden" name="warden" required>
                @foreach($wardens as $warden)
                    @if ($mode!=='create')
                      <option  <?php  if ($warden->id == $orderRecord->wardenId) {echo ' selected ';} ?> VALUE="{{$warden->id}}"> {{$warden->body.', '.$warden->group}}</option>
                    @else
        ...
        <div class="col-md-6 col-lg-9">
            <select class="custom-select d-block w-100" id="adjuster" name="adjuster" required>
                @foreach($adjusters as $adjuster)
                  @if ($mode!=='create')
                    <option <?php if ($adjuster->id == $orderRecord->adjusterId) {echo ' selected ';} ?> VALUE="{{$adjuster->id}}">{{$adjuster->body.', '.$adjuster->group}}</option>
                  @else
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-lg-9">
            <textarea class="form-control" id="brigade_members" name="brigade_members" rows="5"> @if ($mode!=='create') {{trim($teamList)}} @endif </textarea>
            <span class="text-muted"><i>(прізвище, ініціали, група з електробезпеки)</i></span>
        </div>
    </div>
