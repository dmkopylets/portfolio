        ...
        <div class="col-md-6 col-lg-9">
            <select class="custom-select d-block w-100" id="warden" name="warden" required>
                @foreach($wardens as $warden)
                    @if ($mode!=='create')
                      <option  <?php  if ($warden->id == $orderRecord->warden_id) {echo ' selected ';} ?> VALUE="{{$warden->id}}"> {{$warden->body.', '.$warden->group}}</option>
                    @else
        ...
        <div class="col-md-6 col-lg-9">
            <select class="custom-select d-block w-100" id="adjuster" name="adjuster" required>
                @foreach($adjusters as $adjuster)
                  @if ($mode!=='create')
                    <option <?php if ($adjuster->id == $orderRecord->adjuster_id) {echo ' selected ';} ?> VALUE="{{$adjuster->id}}">{{$adjuster->body.', '.$adjuster->group}}</option>
                  @else
    ...
    <div class="row">
        <div class="col-md-3">
            <label for="brigade_members"  style="font-size: 10pt;">з членами бригади:</label>
              ...
              @if ($mode!=='create')
                <?php $brlist='';
                if (isset($array['$brigadeText'])) {
                    var_dump($brigadeText);
                        foreach($brigadeText as $txt1)
                            {
                              $brlist=$brlist.$txt1->body.' '.$txt1->group.', ';
                            }
                        }
                if (isset($array['$engineersText'])) {
                        foreach($engineersText as $txt2)
                            {
