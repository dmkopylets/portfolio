<div class="measure">
    ...
    <table class="table  table-bordered table-sm" style="margin-top:0; margin-bottom: 1px;">
        ...
        <tr valign="middle" align="center" style="line-height: 80%;">
            <th scope="col" width="300px">Дозвіл на підготовку робочих місць і на допуск видав <br/> (посада, прізвище
                або підпис)
            </th>
            <th scope="col" width="120px">Дата, час</th>
            <th scope="col" width="160px">Підпис працівника, який отримав<br/>дозвіл на підготовку робочих<br/>місць і
                на допуск
            </th>
        </tr>
        ...
        @if ($count_meas_row > 0)
            @foreach ($measures_rs as $mrow)
                <tr>
                    ...
                    <td>
                        <button wire:click="editMeasure({{$mrow['id']}})" class="btn btn-primary btn-sm"><i
                                    class="fa fa-pencil fa-fw"></i>{{$mrow['id']}}</button>
                        <button wire:click="$emit('destroyMeasure',{{$mrow['id']}})" class="btn btn-danger btn-sm"><i
        ...
        @endif
        @if($updateMeasure)
            @include('orders.edit.measure.update')
        @else
            @include('orders.edit.measure.create')
        @endif
        </tbody>
