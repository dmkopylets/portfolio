...
//    прописуємо поточну дату в усі календарики
           $("#datetime_work_begin").datetimepicker({ format: dateFormat,locale: 'uk',  date: moment(todayStr, dateFormat)});
       ...
       function brig_engr_checked() {
               ...
               // check-нувся якийсь член бригади
                      ...
                      @foreach($brig_m_arr as $br_member)
                         if (document.getElementById("brigade_member{{$br_member->id}}").checked == true){
                              i1++;
                              text_br_all=text_br_all+', {{$br_member->body.' '.$br_member->group}}';
                              text_br_all_id_to_write_on_db=text_br_all_id_to_write_on_db+',{{$br_member->id}}';
                         }
                      @endforeach
...
//                        //Видалення всіх ком на початку        Приклад : s = s.replace(/^,*/,"");
                      ...
                      if (text_br_all.length > 0 ) {text_engr_all=', '+text_engr_all;}
